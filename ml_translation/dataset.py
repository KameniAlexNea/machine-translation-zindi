...
class ToolKit:
    def __init__(
        self, checkpoint: str = "google-t5/t5-small", max_length: int = 512, prefix=""
    ):
        self.tokenizer = AutoTokenizer.from_pretrained(checkpoint, model_max_length=max_length)
        self.data_collator = DataCollatorForSeq2Seq(
            tokenizer=self.tokenizer, model=checkpoint
        )
        self.metric = evaluate.load("sacrebleu")
        self.model = AutoModelForSeq2SeqLM.from_pretrained(checkpoint, max_length=max_length)
